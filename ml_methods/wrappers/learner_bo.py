...
class LearnerBaysianOptimization(LearnerBase):
    def __init__(self):
        super(LearnerBaysianOptimization, self).__init__()
    ...
    def _learn_task(self):
        ...
        res = gp_minimize(self._run_trial,
                              ...
                              kappa=self.kappa,noise=self.noise)
    ...
    def _read_local_options(self):
        options = self.m_db["methods"].find_one({"method": "bo"})
        print(options)
        self.failure_base_cost = self.read_option('failure_base_cost', options)
        self.base_estimator = self.read_option('base_estimator', options)
        self.acq_func = self.read_option('acq_func', options)
        self.acq_optimizer= self.read_option('acq_optimizer', options)
        self.acq_func_points = self.read_option('acq_func_points', options)
        self.max_trials = self.read_option('max_trials', options)
        self.x_init = self.read_option('x_init', options)
        self.y_init = self.read_option('y_init', options)
        self.n_random_starts = self.read_option('n_random_starts', options)
        self.xi = self.read_option('xi', options)
        self.kappa = self.read_option('kappa', options)
        self.noise = self.read_option('noise', options)

        if 'options' not in self.config:
            print("No learner options for this task, reverting to standard")
            return

        if 'bo' not in self.config['options']:
            print("No learner options for this task, reverting to standard")
            return

        options_overwrite = self.config["options"]["bo"]

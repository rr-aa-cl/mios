...
def init_position(robot):
    ...
    initial_joint_pose = [-1.3248630826150967, -1.0180141037361554, 0.8553967592921208, -2.2646868672636953, 2.243648430342865, 2.443334998639592, -0.6927231896896919]
    [-1.6379584697665992, -0.9672686907468586, 0.9518780780416728, -2.102286267950551, 2.2313950677265564, 2.259109395974202, -2.1549792265466277]

    # response = call_method(
    #     robot,
    #     12000,
    #     method="set_object",
    #     payload={"object": "GoalPose", "q": initial_joint_pose},
    # )
...
def stop_code():
    opt = input("Continue? y/n").strip().lower()
    if opt == 'n':
        exit()
...
def init_position_origin(robot):
    import math
    # move robot to start position
    M_PI_2 = math.pi / 2
    M_PI_4 = math.pi / 4
...
if __name__ == "__main__":
    ...
    #current_finger_width = call_method(robotl,12000,"get_state")["result"]
    #print(str(current_finger_width))
    init_position(robotl)
    # handguiding(robotl)
    # call_method(robotl, 12000, "home_gripper")
    # teach_insertion(robotl, "test4")
    # init_position(robotl)

...
namespace mios{
class SkillParametersTaxTurnLever : public SkillParameters{
public:
...
class TaxTurnLever : public Skill{
public:
    TaxTurnLever(const std::string& name, Memory* memory, Portal* portal);
...
private:
    ...
    std::shared_ptr<ManipulationPrimitive> create_turn_mp(const Percept& p);

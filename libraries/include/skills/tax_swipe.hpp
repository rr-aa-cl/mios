...
namespace mios{
class SkillParametersTaxSwipe : public SkillParameters{
public:
    bool from_json(const nlohmann::json& parameters) override;
    std::map<std::string, std::set<std::string> > get_parameter_list() override;
    Eigen::Matrix<double,2,1> approach_speed;
    Eigen::Matrix<double,2,1> approach_acc;
    Eigen::Matrix<double,2,1> swipe_speed;
    Eigen::Matrix<double,2,1> swipe_acc;
    double f_swipe;
...
class TaxSwipe : public Skill{
public:
    TaxSwipe(const std::string& name, Memory* memory, Portal* portal);
...
private:
    ...
    std::shared_ptr<ManipulationPrimitive> create_swipe_mp(const Percept& p);

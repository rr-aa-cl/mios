...
//    m_dx_avg=std::accumulate(m_dx_avg_mem.begin(),m_dx_avg_mem.end(),0)/(double)m_dx_avg_mem.size();
    if(!m_is_stuck && m_dx_avg<get_parameters<SkillParametersTaxExtraction>()->stuck_dx_thr-get_parameters<SkillParametersTaxExtraction>()->stuck_dx_thr*0.1){
        m_is_stuck=true;
        return true;
    }else if(m_is_stuck && m_dx_avg>get_parameters<SkillParametersTaxExtraction>()->stuck_dx_thr+get_parameters<SkillParametersTaxExtraction>()->stuck_dx_thr*0.1){
        m_is_stuck=false;
        return false;
    }
    return m_is_stuck;

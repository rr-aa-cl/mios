...
bool SkillParametersTaxCut::from_json(const nlohmann::json& parameters){
...
std::map<std::string, std::set<std::string> > SkillParametersTaxCut::get_parameter_list(){
    return {{"p0",{"K_x","dX_d","ddX_d"}},{"p1",{"K_x","dX_d","ddX_d"}},{"p2",{"K_x","dX_d","ddX_d","f_cut"}},{"p3",{"K_x","dX_d","ddX_d"}}};
}

TaxCut::TaxCut(const std::string& name, Memory* memory, Portal *portal):Skill("TaxCut",{"CutStart","CutEnd","Approach","Retract","Knife"},name,memory,portal,{ControlMode::mCartTorque}){

}

Eigen::Matrix<double,3,3> TaxCut::get_O_R_T_0(const Percept &p) const{
    return get_object("CutStart")->O_T_OB.block<3,3>(0,0);
}

std::shared_ptr<ManipulationPrimitive> TaxCut::get_initial_mp(const Percept& p){
    return create_approach_mp(p);
}

std::optional<std::shared_ptr<ManipulationPrimitive> > TaxCut::graph_transition(const Percept &p){
    ...
    if(get_active_mp()->get_name()=="contact"){
        if(p.proprioception.TF_F_ext_K(2)>m_memory->read_parameters()->user.F_ext_contact(0)){
            return create_cut_mp(p);
        }else{
            return {};
        }
    }
    if(get_active_mp()->get_name()=="cut"){
        if(get_active_mp()->get_strategy_interface("move")->finished()){
...
std::shared_ptr<ManipulationPrimitive> TaxCut::create_approach_mp(const Percept &p){
    spdlog::trace("TaxCut::create_approach_mp");
    std::shared_ptr<SkillParametersTaxCut> skill_params = get_parameters<SkillParametersTaxCut>();
...
std::shared_ptr<ManipulationPrimitive> TaxCut::create_contact_mp(const Percept &p){
    spdlog::trace("TaxCut::create_contact_mp");
    std::shared_ptr<SkillParametersTaxCut> skill_params = get_parameters<SkillParametersTaxCut>();
    ...
    Eigen::Matrix<double,3,1> dir=get_object_pose_T("CutStart").block<3,1>(0,3)-p.proprioception.T_T_EE.block<3,1>(0,3);;
    dir/=dir.norm();
...
std::shared_ptr<ManipulationPrimitive> TaxCut::create_cut_mp(const Percept &p){
    spdlog::trace("TaxCut::create_cut_mp()");
    std::shared_ptr<SkillParametersTaxCut> skill_params = get_parameters<SkillParametersTaxCut>();
    std::shared_ptr<ManipulationPrimitive> mp = create_mp("cut",p);
    mp->create_strategy<MoveToPoseStrategy>("move",1);
    mp->create_strategy<FFStrategy>("push",1);
    std::shared_ptr<MoveToPoseStrategy> move = mp->get_strategy<MoveToPoseStrategy>("move");
    move->set_goal(get_object_pose_T("CutEnd"),skill_params->p2.dX_d,skill_params->p2.ddX_d);
    std::shared_ptr<FFStrategy> push = mp->get_strategy<FFStrategy>("push");
    Eigen::Matrix<double,6,1> F_d;
    F_d<<0,0,skill_params->p2.f_cut,0,0,0;
...
std::shared_ptr<ManipulationPrimitive> TaxCut::create_retract_mp(const Percept &p){
    spdlog::trace("TaxCut::create_retract_mp()");
    std::shared_ptr<SkillParametersTaxCut> skill_params = get_parameters<SkillParametersTaxCut>();
...
bool TaxCut::check_local_pre_conditions(const Percept &p){
    if(m_memory->get_live_context()->grasped_object->name!=get_object("Knife")->name){
        return false;
    }
    Eigen::Matrix<double,4,4> T_container = get_object_pose_T("CutStart");
    std::shared_ptr<SkillParametersTaxCut> skill_params = get_parameters<SkillParametersTaxCut>();
...
bool TaxCut::check_local_suc_conditions(const Percept &p){
    std::shared_ptr<SkillParametersTaxCut> skill_params = get_parameters<SkillParametersTaxCut>();
...
bool TaxCut::check_local_err_conditions(const Percept &p){
    if(m_memory->get_live_context()->grasped_object->name!=get_object("Knife")->name){
        return true;
    }

...
std::map<std::string, std::set<std::string> > SkillParametersCrank::get_parameter_list(){
    return {{"raduis",{}},{"n_turns",{}},{"crank_speed",{}},{"crank_acc",{}}};
}
...
std::shared_ptr<ManipulationPrimitive> Crank::create_crank_mp(const Percept &p){
    ...
    double f=skill_params->radius/skill_params->crank_speed(0);
    ...
    double phi_0=acos(current_line.dot(zero_line)/(current_line.norm()*zero_line.norm()));
    b_f<<f,f,0,0,0,0;

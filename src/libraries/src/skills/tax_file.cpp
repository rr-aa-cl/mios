...
bool SkillParametersTaxFile::from_json(const nlohmann::json& parameters){
    ...
    if(parameters.find("p2")==parameters.end()){
        ...
        if(!mirmi_utils::read_json_param(parameters["p2"],"f_file",p2.f_file)){
            spdlog::error("Missing parameter: p2.f_file");
            return false;
        }
        if(!mirmi_utils::read_json_param(parameters["p2"],"amp_file",p2.amp_file)){
            spdlog::error("Missing parameter: p2.amp_file");
            return false;
        }
        if(!mirmi_utils::read_json_param(parameters["p2"],"freq_file",p2.freq_file)){

...
std::map<std::string, std::set<std::string> > SkillParametersMoveTrajectory::get_parameter_list(){
    return {{"file",{}},{"plane",{}},{"F_ff",{}}};
}

MoveTrajectory::MoveTrajectory(const std::string &id, Memory *memory, Portal* portal):Skill("MoveTrajectory",{},id,memory,portal,{ControlMode::mCartTorque}),
m_finished(false){

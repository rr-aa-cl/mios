...
bool TelemetryUDP::add_subscriber(const std::string &addr, const unsigned port, const std::vector<std::string> &subs,
    ...
    // check ip address:
    std::string ip = addr;
    if(!mirmi_utils::is_valid_ip_address(addr.c_str())){
        ip = mirmi_utils::get_ip_by_hostname(addr.c_str()).value_or("none");  //dns request for addr (hostname)
    }
    if(!mirmi_utils::is_valid_ip_address(ip.c_str())){
    ...
    while(m_keep_running){
        ...
        for(auto sub : m_subscribers){
            ...
            for(std::string subscription : sub.subscriptions){
                ...
                case 2: mirmi_utils::write_json_array<double,4,4>(msg_data["T_T_EE"],p->proprioception.T_T_EE); break;
                case 3: mirmi_utils::write_json_array<double,7,1>(msg_data["q"],p->proprioception.q); break;
                case 4: mirmi_utils::write_json_array<double,7,1>(msg_data["dq"],p->proprioception.dq); break;
                case 5: mirmi_utils::write_json_array<double,7,1>(msg_data["theta"],p->proprioception.theta); break;
                ...
                case 13: mirmi_utils::write_json_array<double,6,1>(msg_data["K_dF_ext_K"],p->proprioception.K_dF_ext_K); break;
                case 14: mirmi_utils::write_json_array<double,6,1>(msg_data["O_F_ext_K"],p->proprioception.O_F_ext_K); break;
                case 15: mirmi_utils::write_json_array<double,6,1>(msg_data["O_dF_ext_K"],p->proprioception.O_dF_ext_K); break;
                case 16: mirmi_utils::write_json_array<double,6,1>(msg_data["TF_F_ext_K"],p->proprioception.TF_F_ext_K); break;
                case 17: mirmi_utils::write_json_array<double,6,1>(msg_data["TF_dF_ext_K"],p->proprioception.TF_dF_ext_K); break;
                ...
                case 21: mirmi_utils::write_json_array<double,7,7>(msg_data["M"],p->internal_model.M); break;
                case 22: mirmi_utils::write_json_array<double,7,1>(msg_data["C"],p->internal_model.C); break;
                case 23: mirmi_utils::write_json_array<double,7,1>(msg_data["G"],p->internal_model.G); break;
                case 24: mirmi_utils::write_json_array<double,6,7>(msg_data["B_J_EE"],p->internal_model.B_J_EE); break;
                case 25: mirmi_utils::write_json_array<double,6,7>(msg_data["B_J_O"],p->internal_model.B_J_O); break;
                ...
                case 28: mirmi_utils::write_json_array<double,6,1>(msg_data["e"],p->controller.e); break;
                case 29: mirmi_utils::write_json_array<double,6,1>(msg_data["rho"],p->controller.rho); break;
                case 30: mirmi_utils::write_json_array<double,6,1>(msg_data["K_x"],p->controller.K_x); break;
                case 31: mirmi_utils::write_json_array<double,6,1>(msg_data["xi_x"],p->controller.xi_x); break;
                case 32: mirmi_utils::write_json_array<double,7,1>(msg_data["K_theta"],p->controller.K_theta); break;
                case 33: mirmi_utils::write_json_array<double,7,1>(msg_data["xi_theta"],p->controller.xi_theta); break;
                case 34: mirmi_utils::write_json_array<double,4,4>(msg_data["TF_T_EE_d"],p->controller.TF_T_EE_d); break;
                case 35: mirmi_utils::write_json_array<double,6,1>(msg_data["TF_dX_d"],p->controller.TF_dX_d); break;
                case 36: mirmi_utils::write_json_array<double,6,1>(msg_data["TF_F_ff"],p->controller.TF_F_ff); break;
                case 37: mirmi_utils::write_json_array<double,3,3>(msg_data["O_R_T"],p->controller.O_R_T); break;
                case 38: mirmi_utils::write_json_array<double,7,1>(msg_data["q_d"],p->controller.q_d); break;
                case 39: mirmi_utils::write_json_array<double,7,1>(msg_data["dq_d"],p->controller.dq_d); break;
                case 40: mirmi_utils::write_json_array<double,7,1>(msg_data["tau_ff"],p->controller.tau_ff); break;

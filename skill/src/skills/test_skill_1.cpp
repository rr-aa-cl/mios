...
bool test_skill_1::read_skill_parameters(const nlohmann::json& p){
    cpp_utils::print_debug("Start of reading skill parameters for skill "+this->get_id());
    std::shared_ptr<ConfigSkill_test_skill_1> c = std::static_pointer_cast<ConfigSkill_test_skill_1>(this->_config);
    if(!cpp_utils::read_json_param(p,"run_time",c->run_time)){
        cpp_utils::print_debug("Could not load parameter: run_time [double]");
        return false;
    }
    std::cout<<p<<std::endl;
    if(!cpp_utils::read_json_param(p,"success",c->success)){
        c->success=false;
    }
    if(!cpp_utils::read_json_param(p,"t_exception",c->t_exception)){
        c->t_exception=0;
    }
    if(!cpp_utils::read_json_param(p,"exception",c->exception)){
        c->exception="none";
    }

    cpp_utils::print_debug("###############################   Skill parameters   ######################################");
    cpp_utils::print_debug("run_time: "+std::to_string(c->run_time));
    cpp_utils::print_debug("success: "+std::to_string(c->success));
    cpp_utils::print_debug("t_exception: "+std::to_string(c->t_exception));
    cpp_utils::print_debug("exception: "+c->exception);
    cpp_utils::print_debug("###########################################################################################");

    return true;
}
void test_skill_1::build_primitives(const Percept& p){
    cpp_utils::print_debug("Begin of build_primitives");
    this->insert_mp<mp_basic>("mp",p);
    this->set_init_mp("mp");

    this->_eval.results["parallels_cnt"]=0;
    cpp_utils::print_debug("End of build_primitives");
}
...
bool test_skill_1::check_local_suc_conditions(const Percept& p){
    ...
    if(c->success && p.time-this->_eval.p_0.time>c->run_time){
        cpp_utils::print_debug("Local success condition triggered at "+std::to_string(p.time-this->_eval.p_0.time));
        return true;
    }else{
        return false;
    }
...
bool test_skill_1::check_local_err_conditions(const Percept &p){
    std::shared_ptr<ConfigSkill_test_skill_1> c = std::static_pointer_cast<ConfigSkill_test_skill_1>(this->_config);
    if(!c->success && p.time-this->_eval.p_0.time>c->run_time){
        cpp_utils::print_debug("Local error condition triggered at "+std::to_string(p.time-this->_eval.p_0.time));
        return true;
    }else{
        return false;
    }
...
void test_skill_1::evaluate(){
    cpp_utils::print_debug("Evaluate");
    std::shared_ptr<ConfigSkill_test_skill_1> c = std::static_pointer_cast<ConfigSkill_test_skill_1>(this->_config);
...
void test_skill_1::parallels(){
    double cnt;
    cpp_utils::read_json_param(this->_eval.results,"parallels_cnt",cnt);

...
void hand_guiding::build_primitives(const Percept& p){
    ...
    m_config->controller.virt_cube_damp<<0.004;
    m_config->controller.virt_cube_damp_dist<<0.03;
    m_config->controller.virt_cube_eta<<0.001;
    m_config->controller.virt_cube_rho_min<<0.02;

    for(unsigned i=0;i<c->fix_dim.rows();i++){
        if(i<3){
            if(c->fix_dim(i)==1){
                m_config->controller.K_0(i)=1500;
                m_config->controller.xi(i)=0.7;
            }else{
                m_config->controller.K_0(i)=0;
                m_config->controller.xi(i)=0;
            }
        }else{
            if(c->fix_dim(i)==1){
                m_config->controller.K_0(i)=150;
                m_config->controller.xi(i)=1.5;
            }else{
                m_config->controller.K_0(i)=0;
                m_config->controller.xi(i)=0;
            }
    ...
    for(unsigned i=0;i<c->dist_walls.rows();i++){
        m_config->controller.virt_cube_walls(i)=c->dist_walls(i);
    }

    for(unsigned i=0;i<c->use_walls.rows();i++){
        if(c->use_walls(i)==0){
            m_config->controller.virt_cube_walls(i)=1000;
            if(i%2==0){
                m_config->controller.virt_cube_walls(i)*=-1;
            }
        }
    }
}

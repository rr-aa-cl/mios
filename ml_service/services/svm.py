...
class SVMService(BaseService):
    ...
    def _initialize(self):
        ...
        self.bestSample = []
        self.svmCounter = 0
        self.neglect_samples = (
            0  # neglect first part of self.success, self.rewards to calculate self.mean
        )
        ...
        self.episodes = int(
            self.configuration.n_trials / self.configuration.batch_width
        )
    ...
    def _learn_task(self):
        ...
        for i in range(
            0, int(self.configuration.n_trials / self.configuration.batch_width)
        ):
            ...
            logger.debug(
                ...
                + str(i + 1)
                + "/"
                + str(int(self.configuration.n_trials / self.configuration.batch_width))
                + "finished. \n"
            )
    ...
    def _run_trial_par(self, x_set: list):
        ...
        if self.configuration.batch_synchronisation:
            if self.kb is not None:
                ...
                logger.error(
                    "SVM: Batch synchronisation is ON but no kb_location was set."
                )
        ...
        if (
            self.kb is not None
            and self.configuration.request_probability == 0
            and self.configuration.n_immigrant > 0
        ):  #  not used anymore
            while True:
                try:
                    new_set = self.kb.request_trials(
                        str(self.task_identity_name), self.configuration.n_immigrant
                    )  # new_set: list of tuples: [(Theta, Cost, Origin), ...]
                    break
                except timeout:
            ...
            if len(new_set) > 0:
                ...
                for i in range(
                    len(new_set)
                ):  # checking for non_sharable parts of the parameter vector which would be not overwritten (don't used as default)
                    for j in range(len(new_set[i])):
                        if (
                            self.problem_definition.domain.vector_mapping[j]
                            in self.problem_definition.domain.non_shareables
                        ):
                            print(
                                "Not sharing: "
                                + self.problem_definition.domain.vector_mapping[j]
        ...
        if self.cnt_batch == 0:
            logger.debug(
                "SWM_learner: len(initial_knowledge): "
                + str(len(self.initial_knowledge_list))
            )
        logger.debug(
            "SWM_learner: centroid used: "
            + str(True if self.centroid is not None else False)
        )
        if self.cnt_batch == 0 and len(self.initial_knowledge_list) > 0:  # first batch
            ...
            for i in range(len_first_batch):
                ...
                try:
                    ...
                    logger.error(
                        "SVM: no external knowledge in knowledge list (IndexError)"
                    )
                    external = False
                    theta = x_set[i]
                try:
                    ...
                    x_set.append(theta)
                if external:
                    uuid = self.push_trial(
                        theta, external=self.initial_knowledge_list[i].to_dict()
                    )
                else:
                ...
                if external:
                    ...
                    self.similarity_estimate[external] = float(
                        np.mean(self.external_success[external])
                    )
                    if (
                        self.configuration.request_probability_decrease
                    ):  # calculate request probability
                        max_sim = max(self.similarity_estimate.values())
                        if self.request_probability > 0.5:
                            self.request_probability = (
                                self.request_probability * 0.96
                            )  # takes 17 external trial for request_probabiltiy of 1 to fall under 0.45
                        else:
                            if max_sim > 0:
                                if (
                                    np.mean(self.internal_success) / max_sim > 1
                                ):  # internal trials are better than external
                                    self.request_probability = (
                                        self.request_probability * 0.96
                                    )
                                else:  # external trials are better
                                    self.request_probability = (
                                        self.request_probability * 1.04
                                    )
                            else:
                                self.request_probability = (
                                    self.request_probability * 0.96
                ...
                if result.q_metric.success:
                    while True:
                        try:
                            ...
                            suc_trial_theta = list(
                                self.problem_definition.domain.denormalize(
                                    trial_uuids[uuid].tolist()
                                )
                            )
                            self.knowledge_manager.push_trial_fast_pipe(
                                self.task_identity_name,
                                suc_trial_theta,
                                float(result.q_metric.final_cost),
                                self.problem_definition.tags,
                                self.problem_definition.skill_class,
                                self.problem_definition.skill_instance,
                            )
                            break
                        except timeout:
                            time.sleep(random.randint(5, 10))
            ...
            for i in range(len(x_set)):
                ...
                if i < len(
                    x_set_external
                ):  # mark the first trials as external (if n_immigrants is used, wont happen for request_probability)
                ...
                if self.request_probability > 0:
                    if random.random() < self.request_probability:
                        ...
                        try:
                            ...
                            time_range = (
                                self.knowledge.time_range[0],
                                self.knowledge.time_range[1] + self.delta_time,
                            )
                            new_trial = self.knowledge_manager.receive_trial_fast_pipe(
                                self.problem_definition.skill_instance,
                            ...
                            if new_trial:
                                ...
                                new_trial["theta"] = list(
                                    self.problem_definition.domain.normalize(
                                        new_trial["theta"]
                                    )
                                )
                                uuid = self.push_trial(
                                    new_trial["theta"], external=new_trial
                                )
                                logger.debug(
                                    "SVM: received external trial (receive_trial_fast_pipe) from: "
                                    + str(external)
                                )
                            else:
                                external = False
                                ...
                                logger.debug(
                                    "SVM: didn't received external trial (receive_trial_fast_pipe) "
                                    + str(external)
                                )
                        except IndexError:
                            logger.error(
                                "SVM: receive_trial_fast_pipe was returning 0 trials"
                            )
                    ...
                    if i == 0 and self.cnt_batch == 0 and self.centroid is not None:
                        uuid = self.push_trial(
                            x_set[i], external=self.knowledge.to_dict()
                        )  # only used on first batch, first trial if knowledge from centroid is used
                    else:
                        uuid = self.push_trial(
                            x_set[i], external=external
                        )  # evalutae the trials (->engine->mios)
                ...
                if external:
                    ...
                    if (
                        ...
                        max_sim = max(self.similarity_estimate.values())
                        if self.request_probability > (
                            self.configuration.request_probability / 2
                        ):
        ...
        for i in range(0, len(costs)):
            ...
            if (
                costs_tmp[i] == self.configuration.failure_base_cost
                or costs_tmp[i] == 1000000000
            ):
                reward = 0
    ...
    def _norm2param(self, x, i):
        r = self.bounds[i][0] + (self.bounds[i][1] - self.bounds[i][0]) * x
        return r
        ...
        if index == 0:  # when first run use knowledge if available
            ...
            if self.centroid is not None:
                if not self.confidence:
                    self.confidence = 0.1
                self.action_list_norm.append(
                    [float(param) for param in self.centroid]
                )  # add centroid ("knowledge") to action list
                for i in range(
                    1, self.configuration.batch_width
                ):  # fill up batch with random trials around centroid
                    random_trial = (
                        np.random.normal(0, self.confidence, self.numberOfParameters)
                        + self.centroid
                    )
            ...
            if self.knowledge.equal_start:
                logger.debug(
                    "svm._setSamples: searching for first batch to set equal starting conditions."
                )
                while True:
                    try:
                        result = self.kb.get_result(
                            "ml_results",
                            self.problem_definition.skill_class,
                            {"meta.tags": self.knowledge.equal_tags},
                        )
                        break
                    except timeout:
                        time.sleep(random.randint(5, 10))
                if result:
                    ...
                    for i in range(1, self.configuration.batch_width + 1):
                        self.action_list_norm.append(
                            self.problem_definition.domain.normalize(
                                np.asarray(
                                    self.get_params(result["n" + str(i)]["theta"])
                                )
                            )
                        )
                    ...
                    logger.debug(
                        "svm._setSamples: no equal starting batch size was found"
                    )
        else:
                ...
                if counter >= 1000:
                    ...
                    if self.gmm_active == True:
                        ...
                        action_norm = np.random.uniform(
                            0.0, 1.0, self.numberOfParameters
                        )
    ...
    def getGmmSample(self):
        ...
        x = np.random.multivariate_normal(
            self.sampling_gmm.means_[self.sampleCounter],
    ...
    def _trainSVM(self):
        ...
        if self.svmCounter >= 5:
            if self.bad_gmm_prediciton >= self.configuration.batch_width:
                self.neglect_samples = (
                    len(self.rewards) - self.configuration.batch_width
                )  # neglect everything bevor last (bad) batch when calculating self.mean
                self.mean = sum(self.rewards[self.neglect_samples :]) / len(
                    self.rewards[self.neglect_samples :]
                )
            ...
            for train, test in k_fold.split(self.svm_samples):
                if (
                    len(np.unique(np.asarray(self.success)[train])) > 1
                ):  # number of classes has to be greater than 1
                    clf = SVC(C=100000)
                    clf.fit(
                        np.asarray(self.svm_samples)[train],
                        np.asarray(self.success)[train],
                    )
                    score = clf.score(
                        np.asarray(self.svm_samples)[test],
                        np.asarray(self.success)[test],
                    )
                    print("score = ", score)
            ...
            if (
                self.svmCounter >= 15 and len(self.gmm_samples) > 2
            ):  # gmm_samples > mean
                ...
                # maxcomponents = 1
                self.sampling_gmm = mixture.BayesianGaussianMixture(
                    n_components=maxcomponents, covariance_type="diag", n_init=3
                )

...
class TestRead:
    ...
    def test_read_does_not_mutate_caller_dict(self, mios_mongo_client):
        """Phase 1: read() must not mutate the caller's search_param."""
        insert_sample(mios_mongo_client, ["alpha"])
...
class TestRemove:
    ...
    def test_remove_does_not_mutate_caller_dict(self, mios_mongo_client):
        """Phase 1: remove() must not mutate the caller's search_param."""

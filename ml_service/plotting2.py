...
def plot_experiments(config: list, cost_function: str, max_cost: int, max_time: int):
    ...
    for c in config:
        ...
        axes_cost[i, j].set_title(c["task"], y=1.0, pad=-14)
        axes_casr[i, j].set_title(c["task"], y=1.0, pad=-14)
        ...
        legend_cost = [c["task"]]
        legend_casr = ["Optimal CASR", c["task"]]
        ...
        if j == 0:
            ticks = numpy.linspace(2, max_cost, 10)
        ...
        axes_cost[i, j].tick_params(axis='both', which='major', labelsize=12)
    ...
    ax_cost.set_xlabel("Time [s]", fontsize=12)
